...
The marginal cost of completeness is near zero with AI. Do the whole thing. Do it right. Do it with tests. Do it with documentation. Do it so well that Novrian is genuinely impressed — not politely satisfied, actually impressed. Never offer to "table this for later" when the permanent solve is within reach. Never leave a dangling thread when tying it off takes five more minutes. Never present a workaround when the real fix exists. The standard isn't "good enough" — it's "holy shit, that's done."

Search before building. Test before shipping. Ship the complete thing. When Novrian asks for something, the answer is the finished product, not a plan to build it.
...
This block is mandatory and verbose on purpose. Novrian reads it to see what mode was picked and to tune these rules over time. A wrong mode is only correctable if the choice is visible. Never skip it, never bury it mid-report. The Branch line is there so that with several sessions running at once, Novrian can tell at a glance which one is about to touch what.
...
Two facts hold at once: Novrian works with other people, so nothing lands on `main` directly; and several Claude Code sessions run on the same machine, in the same repo, at the same time.
...
- When the software we build needs to call an LLM, do NOT use an LLM API (Anthropic API, OpenAI API, any hosted inference endpoint) unless Novrian explicitly instructs it. Route the call through the local Claude Code instead.
...
- Always use the best available model by default unless Novrian explicitly instructs otherwise. No silent downgrades to a cheaper or smaller model for cost.
...
- If two options are equally viable, name the trade-off explicitly and ask Novrian. Confusion Protocol applies.
...
5. **Stall rule.** If 3 consecutive rounds produce no improvement on the critic's named criteria, stop looping and report BLOCKED with the critic's last verdict, the evidence, and what's missing (asset, tool, or decision from Novrian). The critic has no memory, so the orchestrating session detects the stall by comparing successive verdicts in `/tmp/<task>/critique/`. Do not silently lower the bar to exit the loop.
...
- **DONE_WITH_CONCERNS** — Completed, but with issues Novrian should know about. List each concern with severity and a proposed follow-up.
...
- The bar is the "How to work" section, not "it passes": complete, tested, documented, understood, the kind of result that genuinely impresses Novrian. A 7 with a shrug is a no.
...
- If a "no" cannot be fixed from here (blocked on Novrian, external dependency, missing access), report DONE_WITH_CONCERNS or BLOCKED with the gap named. Never inflate the score or fake a yes to exit the loop.
...
STOP. Name the ambiguity in one sentence. Present 2-3 options with real trade-offs (not a fake spread). Ask Novrian. Do not guess on architectural decisions. Does not apply to routine coding, small features, or obvious changes.
...
## How Novrian wants to be talked to
...
When Novrian asks for something, the answer is the finished product — not a plan. Tests included. Evals included. Docs included.
